...
exports.createOrUpdateWarehouse = async (req, res) => {
  ...
  const { warehouseName, warehouseType, address, email, poc, stock,phone } = req.body;
    ...
    if (id) {
      try {
        ...
        return {
          success: false,
          statusCode: 400,
          message: "Could not create warehouse" + error,
        };
      }
    } else {
      try {
    ...
    if (id) {
      try {
        ...
        return {
          success: false,
          statusCode: 400,
          message: "Could not get warehouse" + error,
        };
      }
    } else {
      try {
  ...
  try {
    if (id) {
      ...
      return {
        success: false,
        statusCode: 400,
        message: "Could not delete warehouser" + error,
      };
    }
  } catch (error) {
    return {
  ...
  try {
    if (id) {
      ...
      return {
        success: true,
        statusCode: 200,
        message: "status updated warehouse successfully",
      };
    } else {
      return {
        success: false,
        statusCode: 400,
        message: "Could not status update warehouser",
      };
    }
  } catch (error) {
    return {
